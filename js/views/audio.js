import { $, $$, esc, shuffle, sleep, isTyping, lengthClass } from '../utils.js';
import { Store } from '../store.js';
import { TTS } from '../tts.js';
import { setTitle } from '../shell.js';
import { isModalOpen } from '../modal.js';
import { onLeave, go } from '../router.js';
import { resolveWordSet } from './flashcards.js';

const DEFAULTS = { repeats: 3, gap: 0.8, wordGap: 1.6, sayMeaning: false, loop: false, shuffle: false };
...
export function viewAudio(el, { id, parts }) {
  ...
  const st = { list: cfg.shuffle ? shuffle(set.words) : [...set.words], i: 0, rep: 0, done: 0, playing: false, token: 0 };
  ...
  const draw = () => {
    ...
    el.innerHTML = `
      <div class="fc-wrap">
        ...
        <div class="card au-stage" id="auStage">
  ...
  const run = async () => {
    ...
    while (st.playing && token === st.token) {
      ...
      // Sang từ tiếp
      if (st.i + 1 < st.list.length) { st.i++; st.done = 0; updateStage(); }
      else if (cfg.loop) { st.i = 0; st.done = 0; if (cfg.shuffle) st.list = shuffle(st.list); updateStage(); }
  ...
  const jump = d => { const was = st.playing; pause(); st.i = (st.i + d + st.list.length) % st.list.length; updateStage(); if (was) play(); };
